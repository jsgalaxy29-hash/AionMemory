...
{
    private static readonly JsonSerializerOptions ModuleSerializerOptions = CreateModuleSerializerOptions();
    private readonly ILLMProvider _provider;
    ...
    public async Task<ModuleDesignResult> GenerateModuleAsync(ModuleDesignRequest request, CancellationToken cancellationToken = default)
    {
        ...
        if (structured.IsValid && !string.IsNullOrWhiteSpace(LastGeneratedJson))
        {
            try
            {
                var parsedModule = JsonSerializer.Deserialize<S_Module>(LastGeneratedJson, ModuleSerializerOptions);
    ...
    private static JsonSerializerOptions CreateModuleSerializerOptions()
    {
