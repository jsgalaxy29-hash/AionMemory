...
public sealed class MemoryAnalyzer : IMemoryAnalyzer
{
    ...
    private static readonly JsonSerializerOptions ParsingOptions = CreateParsingOptions();
    ...
    private static JsonSerializerOptions CreateParsingOptions()
    {
        var options = new JsonSerializerOptions(JsonSerializerDefaults.Web);
        options.Converters.Add(new JsonStringEnumConverter());
        return options;
    ...
    private static bool TryParse(string? raw, out MemoryAnalysisResult result)
    {
        ...
        try
        {
            ...
            if ((topics.Count == 0 || links.Count == 0 || explanation is null)
                && JsonSerializer.Deserialize<MemoryAnalysisPayload>(cleaned, ParsingOptions) is { } payload)
            {
                if (topics.Count == 0)
                {
                    topics = payload.ToTopics();
                }

                if (links.Count == 0)
                {
                    links = payload.ToLinks();
                }

                explanation ??= payload.Explanation?.ToExplanation();
            }
    ...
    private sealed record MemoryAnalysisPayload(
        string? Summary,
        IReadOnlyCollection<MemoryTopicPayload>? Topics,
        IReadOnlyCollection<MemoryLinkPayload>? Links,
        MemoryExplanationPayload? Explanation)
    {
        public IReadOnlyCollection<MemoryTopic> ToTopics()
            => Topics?
                .Where(topic => !string.IsNullOrWhiteSpace(topic.Name))
                .Select(topic => new MemoryTopic(
                    topic.Name!.Trim(),
                    (topic.Keywords ?? Array.Empty<string>())
                        .Where(keyword => !string.IsNullOrWhiteSpace(keyword))
                        .Select(keyword => keyword.Trim())
                        .ToArray()))
                .ToArray()
                ?? Array.Empty<MemoryTopic>();

        public IReadOnlyCollection<MemoryLinkSuggestion> ToLinks()
            => Links?
                .Select(link => new
                {
                    FromId = TryParseGuid(link.FromId),
                    ToId = TryParseGuid(link.ToId),
                    link.Reason,
                    link.FromType,
                    link.ToType,
                    link.Explanation
                })
                .Where(link => link.FromId.HasValue && link.ToId.HasValue && !string.IsNullOrWhiteSpace(link.Reason))
                .Select(link => new MemoryLinkSuggestion(
                    link.FromId!.Value,
                    link.ToId!.Value,
                    link.Reason!.Trim(),
                    link.FromType,
                    link.ToType,
                    link.Explanation?.ToExplanation()))
                .ToArray()
                ?? Array.Empty<MemoryLinkSuggestion>();
    }

    private sealed record MemoryTopicPayload(string? Name, IReadOnlyCollection<string>? Keywords);

    private sealed record MemoryLinkPayload(
        string? FromId,
        string? ToId,
        string? Reason,
        string? FromType,
    ...
    private sealed record MemoryExplanationPayload(
        IReadOnlyCollection<MemorySourcePayload>? Sources,
        IReadOnlyCollection<MemoryRulePayload>? Rules)
    {
        public MemoryAnalysisExplanation? ToExplanation()
        {
            var sources = Sources?
                .Select(source => new
                {
                    RecordId = TryParseGuid(source.RecordId),
                    source.Title,
                    source.SourceType,
                    source.Snippet
                })
                .Where(source => source.RecordId.HasValue)
                .Select(source => new MemoryAnalysisSource(
                    source.RecordId!.Value,
                    source.Title ?? string.Empty,
                    source.SourceType ?? string.Empty,
                    source.Snippet ?? string.Empty))
                .ToArray()
                ?? Array.Empty<MemoryAnalysisSource>();

            var rules = Rules?
                .Where(rule => !string.IsNullOrWhiteSpace(rule.Code) && !string.IsNullOrWhiteSpace(rule.Description))
                .Select(rule => new MemoryAnalysisRule(rule.Code!.Trim(), rule.Description!.Trim()))
                .ToArray()
                ?? Array.Empty<MemoryAnalysisRule>();

            return sources.Length == 0 && rules.Length == 0
                ? null
                : new MemoryAnalysisExplanation(sources, rules);
        }
    }

    private sealed record MemorySourcePayload(string? RecordId, string? Title, string? SourceType, string? Snippet);

    private sealed record MemoryRulePayload(string? Code, string? Description);
