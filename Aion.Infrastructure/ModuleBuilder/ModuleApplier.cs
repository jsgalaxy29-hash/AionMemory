...
public sealed class ModuleApplier : IModuleApplier
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var tableSpec in spec.Tables)
        {
            ...
            EnsureAdded(fieldSync.AddedFields);
            ...
            var addedViews = UpsertViews(table, tableSpec.Views);
            EnsureAdded(addedViews);
    ...
    public async Task<ModuleSchemaVersion> RollbackPublicationAsync(string moduleSlug, CancellationToken cancellationToken = default)
    {
        ...
        var fallbackCandidates = await _db.ModuleSchemaVersions
    ...
    private static IReadOnlyList<SViewDefinition> UpsertViews(STable table, IEnumerable<ViewSpec> views)
    {
        var added = new List<SViewDefinition>();
    ...
    private void EnsureAdded<TEntity>(IEnumerable<TEntity> entities) where TEntity : class
    {
