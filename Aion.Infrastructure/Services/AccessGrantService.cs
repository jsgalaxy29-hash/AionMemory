...
public sealed class AccessGrantService : IAccessGrantService
{
    ...
    private async Task<Permission?> FindExistingPermissionAsync(
        ...
        CancellationToken cancellationToken)
    {
        var candidates = await _db.Permissions
            .AsNoTracking()
            .Include(p => p.Scope)
        ...
        return candidates.FirstOrDefault(p => string.Equals(p.Scope.FieldName, scope.FieldName, StringComparison.OrdinalIgnoreCase));
    }
