...
public sealed class AutomationRuleEngine : IAutomationRuleEngine
{
    ...
    private async Task<string> CreateNoteAsync(AutomationAction action, AutomationEvent automationEvent, CancellationToken cancellationToken)
    {
        ...
        var links = BuildNoteLinks(automationEvent);
    ...
    private async Task<string> ScheduleReminderAsync(AutomationAction action, AutomationEvent automationEvent, CancellationToken cancellationToken)
    {
        ...
        var evt = new S_Event
        {
            ...
            IsCompleted = false,
            Links = BuildEventLinks(automationEvent)
        };
    ...
    private static List<J_Note_Link> BuildNoteLinks(AutomationEvent automationEvent)
    {
        var links = new List<J_Note_Link>();
        if (TryGetGuid(automationEvent.Payload, "recordId", out var recordId))
        {
            links.Add(new J_Note_Link
            {
                TargetType = "Record",
                TargetId = recordId
            });
    ...
    private static List<J_Event_Link> BuildEventLinks(AutomationEvent automationEvent)
    {
        var links = new List<J_Event_Link>();
        if (TryGetGuid(automationEvent.Payload, "recordId", out var recordId))
        {
            links.Add(new J_Event_Link
            {
                TargetType = "Record",
                TargetId = recordId
            });
    ...
    private async Task<List<S_AutomationRule>> LoadRulesAsync(AutomationEvent automationEvent, CancellationToken cancellationToken)
    {
        var query = _db.AutomationRules
            ...
            .Include(r => r.Actions)
            .Where(r => r.IsEnabled && r.Trigger == automationEvent.Trigger);
        ...
        return rules
            .Where(r => string.Equals(r.TriggerFilter, automationEvent.Name, StringComparison.OrdinalIgnoreCase))
            .OrderBy(r => r.Name)
            .ThenBy(r => r.Id)
            .ToList();
    }
    ...
    {
        var matches = await _db.AutomationExecutions.AsNoTracking()
