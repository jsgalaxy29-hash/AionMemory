...
public sealed class AionDataEngine : IAionDataEngine, IDataEngine
{
    ...
    public Task<F_Record> InsertAsync(Guid tableId, string dataJson, CancellationToken cancellationToken = default)
        => InsertAsync(tableId, new Dictionary<string, object?>(ParseJsonPayload(dataJson), StringComparer.OrdinalIgnoreCase), cancellationToken);
    ...
    public Task<F_Record> UpdateAsync(Guid tableId, Guid id, string dataJson, CancellationToken cancellationToken = default)
        => UpdateAsync(tableId, id, new Dictionary<string, object?>(ParseJsonPayload(dataJson), StringComparer.OrdinalIgnoreCase), cancellationToken);
    ...
    public async Task<IEnumerable<F_Record>> QueryAsync(Guid tableId, QuerySpec? spec = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            results = await ApplyOrderingAsync(results, table, spec, cancellationToken).ConfigureAwait(false);

            if (spec.Skip.HasValue)
            {
                results = results.Skip(spec.Skip.Value).ToList();
            }

            if (spec.Take.HasValue)
            {
                results = results.Take(spec.Take.Value).ToList();
            }
...
SELECT RecordId,
       ...
       snippet(RecordSearch, 2, $before, $after, ' â€¦ ', $tokens) AS Snippet,
       Content
FROM RecordSearch
...
""";
    ...
    private static string BuildFallbackSnippet(string content, string before, string after)
    {
        ...
        const int limit = 160;
        var snippet = content.Length <= limit ? content : content[..limit] + "â€¦";
    ...
    private async Task<List<F_Record>> ApplyOrderingAsync(IReadOnlyList<F_Record> records, STable table, QuerySpec spec, CancellationToken cancellationToken)
    {
        if (records.Count == 0)
        {
            return new List<F_Record>();
        }
        ...
        var indexes = await _db.RecordIndexes
            .AsNoTracking()
            .Where(i => i.TableId == table.Id && i.FieldName == field.Name && recordIds.Contains(i.RecordId))
            .ToListAsync(cancellationToken)
            .ConfigureAwait(false);
        ...
        return field.DataType switch
        {
            FieldDataType.Number or FieldDataType.Decimal => spec.Descending
                ? records.OrderByDescending(r => lookup.TryGetValue(r.Id, out var index) ? index.NumberValue : null).ThenByDescending(r => r.CreatedAt).ToList()
                : records.OrderBy(r => lookup.TryGetValue(r.Id, out var index) ? index.NumberValue : null).ThenByDescending(r => r.CreatedAt).ToList(),
            FieldDataType.Boolean => spec.Descending
                ? records.OrderByDescending(r => lookup.TryGetValue(r.Id, out var index) ? index.BoolValue : null).ThenByDescending(r => r.CreatedAt).ToList()
                : records.OrderBy(r => lookup.TryGetValue(r.Id, out var index) ? index.BoolValue : null).ThenByDescending(r => r.CreatedAt).ToList(),
            FieldDataType.Date or FieldDataType.DateTime => spec.Descending
                ? records.OrderByDescending(r => lookup.TryGetValue(r.Id, out var index) ? index.DateValue : null).ThenByDescending(r => r.CreatedAt).ToList()
                : records.OrderBy(r => lookup.TryGetValue(r.Id, out var index) ? index.DateValue : null).ThenByDescending(r => r.CreatedAt).ToList(),
            _ => spec.Descending
                ? records.OrderByDescending(r => lookup.TryGetValue(r.Id, out var index) ? index.StringValue : null).ThenByDescending(r => r.CreatedAt).ToList()
                : records.OrderBy(r => lookup.TryGetValue(r.Id, out var index) ? index.StringValue : null).ThenByDescending(r => r.CreatedAt).ToList()
        };
    }
    ...
    private static void ValidateFieldValue(SFieldDefinition field, object? value)
    {
        ...
        switch (field.DataType)
        {
            ...
            case FieldDataType.Tags:
                ...
                {
                    throw new InvalidOperationException($"Field '{field.Name}' expects a list of tags");
                }
                break;
    ...
    private static object? NormalizeFieldValue(SFieldDefinition field, object? value)
    {
        ...
        return field.DataType switch
        {
            FieldDataType.Text or FieldDataType.Note or FieldDataType.Json or FieldDataType.File
                => NormalizeStringValue(field, value),
            FieldDataType.Tags
                => NormalizeTagsValue(field, value),
        ...
        foreach (var field in table.Fields)
        {
            ...
            switch (field.DataType)
            {
                ...
                case FieldDataType.Date or FieldDataType.DateTime:
                    ...
                    break;
                case FieldDataType.Tags:
                    index.StringValue = value is IEnumerable<string> tags
                        ? string.Join(' ', tags)
                        : value.ToString();
                    break;
                default:
    ...
    private static List<string> NormalizeTagsValue(SFieldDefinition field, object value)
    {
        if (value is JsonElement element)
        {
    ...
    private static int EnsureBasicViews(STable table)
    {
        ...
        if (table.Views.Count == 0)
        {
            table.Views.Add(new SViewDefinition
            {
                ...
                Description = "Vue par dÃ©faut gÃ©nÃ©rÃ©e automatiquement",
                QueryDefinition = "{}",
                Visualization = "table",
                IsDefault = true
            });
