...
public sealed class SemanticSearchService : ISearchService
{
    ...
    private async Task EnsureKeywordIndexesAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (await ShouldApplyMigrationsAsync(cancellationToken).ConfigureAwait(false))
            {
    ...
    private async Task<bool> ShouldApplyMigrationsAsync(CancellationToken cancellationToken)
    {
        if (await TableExistsAsync("__EFMigrationsHistory", cancellationToken).ConfigureAwait(false))
        {
            return true;
        }

        var hasExistingSchema = await TableExistsAsync("Modules", cancellationToken).ConfigureAwait(false)
            || await TableExistsAsync("Records", cancellationToken).ConfigureAwait(false)
            || await TableExistsAsync("Notes", cancellationToken).ConfigureAwait(false)
            || await TableExistsAsync("Files", cancellationToken).ConfigureAwait(false);
...
SELECT {idColumn},
       ...
       snippet({tableName}, {snippetColumn}, $before, $after, ' â€¦ ', $tokens) AS Snippet
FROM {tableName}
WHERE {tableName} MATCH $query
ORDER BY Score DESC;
""";

            AddParameter(command, "$query", NormalizeMatchQuery(query));
    ...
    private static string NormalizeMatchQuery(string query)
    {
