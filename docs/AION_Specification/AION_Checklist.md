...
> Document de suivi.
> Cette checklist melange etat observe, objectifs et points a verifier ; elle doit etre lue comme un support de pilotage, pas comme une source de verite unique sur l'implementation.
...
- [ ] Supprimer la dépendance directe sur Aion.AI ou isoler les orchestrateurs via interfaces domaine. (le csproj référence encore `Aion.AI` et `Aion.Domain`)
...
- [ ] Prévoir thèmes/réactivité et ergonomie mobile ; étendre les tests UI bUnit existants. (tests bUnit déjà présents, couverture à enrichir)
