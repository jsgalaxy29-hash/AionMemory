...
public class MemoryAnalyzerTests
{
    ...
    public async Task AnalyzeAsync_parses_structured_json()
    {
        ...
        var payload = $$"""
        {
            "summary": "Synthese courte",
            "topics": [ { "name": "Projet", "keywords": ["planning", "roadmap"] } ],
            "links": [ { "fromId": "{{recordA.Id}}", "toId": "{{recordB.Id}}", "reason": "Meme projet", "fromType": "note", "toType": "event" } ]
        }
        """;
        ...
        Assert.Equal("Synthese courte", result.Summary);
        ...
        Assert.Equal("Meme projet", link.Reason);
    }
