...
{
    public static IServiceCollection AddAppHostUiDefaults(this IServiceCollection services)
    {
        ...
        services.AddSingleton<IOfflineActionQueue, FakeOfflineActionQueue>();
        services.AddSingleton<IRecordQueryService, FakeRecordQueryService>();
        services.AddSingleton<IModuleViewService>(new FakeModuleViewService(Array.Empty<STable>()));
        services.AddSingleton<INoteService, FakeNoteService>();
        services.AddSingleton<IAgendaService, FakeAgendaService>();
...
{
    public Task<AuthorizationResult> AuthorizeAsync(Guid userId, PermissionAction action, PermissionScope scope, CancellationToken cancellationToken = default)
...
internal sealed class FakeCurrentUserService : ICurrentUserService
{
    ...
    public FakeCurrentUserService()
        : this(Guid.Parse("00000000-0000-0000-0000-000000000001"))
    {
    }
...
internal sealed class FakeOfflineActionQueue : IOfflineActionQueue
{
    public Task<OfflineRecordAction> EnqueueAsync(OfflineRecordAction action, CancellationToken cancellationToken = default)
        => Task.FromResult(action);

    public Task<IReadOnlyCollection<OfflineRecordAction>> GetPendingAsync(CancellationToken cancellationToken = default)
        => Task.FromResult<IReadOnlyCollection<OfflineRecordAction>>(Array.Empty<OfflineRecordAction>());

    public Task<IReadOnlyCollection<OfflineRecordAction>> GetPendingAsync(Guid tableId, CancellationToken cancellationToken = default)
        => Task.FromResult<IReadOnlyCollection<OfflineRecordAction>>(Array.Empty<OfflineRecordAction>());

    public Task<bool> HasPendingAsync(Guid tableId, Guid recordId, CancellationToken cancellationToken = default)
        => Task.FromResult(false);

    public Task MarkAppliedAsync(Guid actionId, CancellationToken cancellationToken = default)
        => Task.CompletedTask;

    public Task MarkFailedAsync(Guid actionId, string reason, CancellationToken cancellationToken = default)
        => Task.CompletedTask;
}
