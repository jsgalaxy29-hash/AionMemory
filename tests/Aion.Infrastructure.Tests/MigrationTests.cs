...
public class MigrationTests
{
    ...
    public async Task Migrations_apply_and_automation_log_is_inserted()
    {
        ...
        var orchestrator = new AutomationOrchestrator(context, new StubAutomationRuleEngine(rule.Id), NullLogger<AutomationOrchestrator>.Instance);
    ...
    private sealed class StubAutomationRuleEngine : IAutomationRuleEngine
    {
        private readonly Guid _ruleId;

        public StubAutomationRuleEngine(Guid ruleId)
        {
            _ruleId = ruleId;
        }
        ...
        public Task<IReadOnlyCollection<AutomationExecution>> ExecuteAsync(AutomationEvent automationEvent, CancellationToken cancellationToken = default)
            => Task.FromResult<IReadOnlyCollection<AutomationExecution>>(
            [
