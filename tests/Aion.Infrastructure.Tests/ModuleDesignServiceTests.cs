...
public class ModuleDesignServiceTests
{
    ...
    public async Task DesignAndApply_creates_module_from_ai_spec()
    {
        ...
        var result = await designer.DesignAndApplyAsync(new Aion.Domain.ModuleBuilder.ModuleDesignRequest
        {
        ...
        Assert.Contains(result.Design.Sources, source => source.Url?.ToString() == "https://schema.org/Thing");
