...
public class IntentRecognizerIntegrationTests
{
    ...
    public async Task IntentRecognizer_handles_wrapped_json_payload()
    {
        ...
        var recognizer = new BasicIntentDetector(provider, NullLogger<BasicIntentDetector>.Instance, new TestOptionsMonitor<AionAiOptions>(new AionAiOptions()), new NoopOperationScopeFactory());
